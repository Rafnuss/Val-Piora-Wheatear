...
```{r}
file <- paste0("figure_print/marginal_animation/marginal_animation_",params$gdl_id,".gif")
if(file.exists(file)){
  knitr::include_graphics(file)
}
```
...
```{r}
file <- paste0("figure_print/expected_use/post_breeding_",params$gdl_id,".png")
if(file.exists(file)){
  knitr::include_graphics(file)
}
file <- paste0("figure_print/expected_use/pre_breeding_",params$gdl_id,".png")
if(file.exists(file)){
  knitr::include_graphics(file)
}
```
...
speed_df <- data.frame(
  ...
  gs = abs(grl$gs[edge]),
  ws = abs(grl$ws[edge]),
  sta_id_s = rep(head(grl$sta_id,-1), ncol(edge)),
  sta_id_t = rep(tail(grl$sta_id,-1), ncol(edge)),
  flight_duration = rep(head(grl$flight_duration,-1), ncol(edge))
)
